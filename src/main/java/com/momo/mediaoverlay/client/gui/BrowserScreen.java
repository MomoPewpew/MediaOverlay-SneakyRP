...
public class BrowserScreen extends GuiScreen {
    ...
    @Override
    public void initGui() {
        MediaOverlay.INSTANCE.hudBrowser = null;
    ...
    public void onGuiClosed() {
        ...
        if(!MediaOverlay.INSTANCE.hasBackup() && browser != null)
            browser.close();
    ...
    //Called by MediaOverlay when the current browser's URL changes.
    ...
    protected void actionPerformed(GuiButton src) {
        ...
        if(src.id == 0)
            ...
            String fixedURL = MediaOverlay.INSTANCE.getAPI().punycode(url.getText());
            browser.loadURL(fixedURL);
        } else if(src.id == 3) {
            MediaOverlay.INSTANCE.setBackup(this);
            mc.displayGuiScreen(null);
        } else if(src.id == 4) {
